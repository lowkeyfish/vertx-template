...
public record JWTConfigItem(String secret, String algorithm, Integer expiresInSeconds) {
    @Override
    ...
    public Integer expiresInSeconds() {
        CheckUtils.notNull(
                expiresInSeconds, new SystemException("配置文件缺少配置项[jwt.*.expiresInSeconds]", ErrorType.CONFIG_ERROR));
